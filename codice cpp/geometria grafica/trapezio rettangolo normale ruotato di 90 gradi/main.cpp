...
{   
    ...
    l = y - 1; //imposta il numero di spazi da stampare per ogni riga 
    
    for(i = 0; i < x; i++){ //ciclo che scorre le righe 
        
        cout << endl;
        
        for(j = 0; j < l; j++){ //prima stampa gli spazi
            cout << "  ";
        }
        for(j; j < y; j++){ //poi stampa i punti
            cout << "* ";
        }
        
        l = (l - 1) * (l > 0); //diminuise di 1 il numero di spazi da fare e se scende sotto zero riporta l = 0
